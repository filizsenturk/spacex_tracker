...
class _RocketsDetailsBodyState extends State<RocketsDetailsBody> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
        ...
        child: SingleChildScrollView(
          child:
          BlocBuilder<RocketCubit,int? >(builder: (context, state) {
  ...
  Widget buildRocketDetail( context,index) {
    ...
    return Column(
      ...
      DetailInfoRow(
          ...
          result:query.active==null?"": query.active.toString()),
   // //  DetailInfoRow(
   // //      title: S.current.boosters,
   // //      result: query.boosters.toString()),
       DetailInfoRow(
           title: S.current.successRate,
           result: query.successRatePct.toString()),
   // // DetailInfoRow(
   // //      title: S.current.engine,
   // //      result: query.secondStage!.engines.toString()),
       DetailInfoRow(
           title: S.current.costPerLunch,
           result: query.costPerLaunch.toString()),
       DetailInfoRow(
           title: S.current.landingLegsMaterial,
           result: query.landingLegs==null?"-" :query.landingLegs!.material==null ?"-": query.landingLegs!.material.toString()),
       DetailInfoRow(
           title: S.current.landingLegsNumber,
           result: query.landingLegs==null ? "-":query.landingLegs!.number.toString()),
    //   DetailInfoRow(
    //       title: S.current.firstFlight,
    //       result: query.firstFlight.toString()),
       DetailInfoRow(
           title: S.current.reUseable,
           result: query.firstStage==null  ? "-":
           query.firstStage!.reusable.toString()),
      ],
    );
  }
  ...
  buildBackIcon() {
    return Padding(
      ...
      child: Row(
        children: [
          IconButton(onPressed: onTapped, icon: Icon(Icons.arrow_back_ios,color: Colors.black26,)),
        ],
      ),
    );
  }
  ...
  buildImage() {
    return Container(
        width: 100.w,
        child: Image.asset("assets/images/hangar.jpg",));
  }
}